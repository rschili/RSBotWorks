...
public partial class WernstromService
{
    ...
    // Used for daily diss at 20:00
    public HomeAssistantPlugin? HAPlugin { get; internal set; }

    private async Task PerformEveningOperation()
    {
        if (!IsRunning)
        {
            Logger.LogWarning("Attempted to perform evening operation while service is not running.");
            return;
        }

        if(HAPlugin == null)
        {
            Logger.LogWarning("HomeAssistantPlugin is not set, skipping evening operation.");
            return;
        }

        var healthData = await HAPlugin.GetRawHealthDataAsync().ConfigureAwait(false);
        if (healthData?.LastUpdated == null)
        {
            Logger.LogWarning("Health data or LastUpdated is null, skipping evening operation.");
            return;
        }

        var timeDifference = DateTime.UtcNow - healthData.LastUpdated;
        if (timeDifference > TimeSpan.FromHours(8))
        {
            Logger.LogWarning("Health data is too old ({Hours:F1} hours), skipping evening operation.", timeDifference.TotalHours);
            return;
        }

        var channel = await DiscordClient.GetChannelAsync(225303764108705793ul).ConfigureAwait(false);
        var user = await channel.GetUserAsync(263959699764805642ul).ConfigureAwait(false);
        if (channel == null || channel is not ITextChannel textChannel || user == null)
        {
            Logger.LogError("Failed to get text channel or user for evening operation");
            return;
        }

        var cachedChannel = TextChannels.FirstOrDefault(c => c.Id == channel.Id);
        if (cachedChannel == null)
        {
            cachedChannel = new JoinedTextChannel<ulong>(channel.Id, channel.Name, await GetChannelUsers(channel).ConfigureAwait(false));
        ...
        var cachedUser = cachedChannel.GetUser(user.Id);
        if (cachedUser == null)
        {
        ...
        string prompt = $"""
            {GENERIC_INSTRUCTION}
            Es ist Abend, das letzte Update von {cachedUser.SanitizedName}'s Gesundheitsdaten war: '{healthData.LastUpdated.ToRelativeToNowLabel()}'.
            Er ist heute {healthData.Steps} Schritte gelaufen, ist {healthData.StandingHours} Stunden gestanden und hat {healthData.WalkingDistance:F1} km zurückgelegt.
            Verfasse eine kurze Nachricht, die die Gesundheitsdaten enthält und urteile streng darüber.
            Wenn die Daten schlecht sind, geize nicht mit Spott und Beleidigungen.
            Sprich ihn in der Nachricht über die Syntax [[{cachedUser.SanitizedName}]] an, damit er ein highlight bekommt.
            Du bekommst zusätzlich die paar letzten Nachrichten als Kontext.
            Nachrichten anderer Nutzer in der Chathistorie bekommst du in folgendem Format übergeben: `[Zeit] [[Name]]: Nachricht`.
            """;

        var liveHistory = await textChannel.GetMessagesAsync(5).FlattenAsync().ConfigureAwait(false);
        var sortedHistory = liveHistory.OrderBy(m => m.CreatedAt).ToList();
        ...
        if (string.IsNullOrEmpty(response))
        {
            Logger.LogWarning("Got an empty response to Evening Message");
            return;
        }
